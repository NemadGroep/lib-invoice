...
from Utilities import clean_special_characters

class Invoice:
    def __init__(self, Output, Config, Json, uid, adress, message, business, subject, text, pdf):
        self.Output = Output
        self.Config = Config
        self.Json = Json
        self.uid = uid
    ...
    def configure_kvpairs(self, kv_pairs):
    ...
    def additional_kv_pairs(self, Json, Dates):
        """Adds additional key-value pairs to the invoice."""
        self.kvpairs['Creation_date'] = Dates.date()
        self.kvpairs['Creation_time'] = Dates.time()
        self.kvpairs['Timestamp'] = Dates.timestamp()
        self.kvpairs['Debtor_code'] = (Json.read(self.Config.get('path_three'))).get(self.kvpairs['Debtor_name'], {}).get('debtor_code')
        if self.kvpairs['Debtor_code'] is None: raise MissingValueError("Debtor code is None")
        self.kvpairs['Debtor_international_location_number'] = (Json.read(self.Config.get('path_three'))).get(self.kvpairs['Debtor_name'], {}).get('illnr')
        self.kvpairs['Partner_country'] = (Json.read(self.Config.get('path_four'))).get(self.kvpairs['Partner_country'])
    ...
    def configure_tax_(self, Json):
        """Configures the tax for the invoice."""
        eu_countries = (Json.read(self.Config.get('path_seven'))).get('EU_country_abbreviations', [])
        if self.kvpairs['Partner_country'] == 'NL':
            self.kvpairs['Tax_qualifier'] = (Json.read(self.Config.get('path_five'))).get('NL', {}).get(str(int(self.kvpairs['Tax_percent'] if self.kvpairs['Tax_percent'] is not None else 0)))
            ...
            self.kvpairs['Tax_qualifier'] = (Json.read(self.Config.get('path_five'))).get('EU', {}).get(str(int(self.kvpairs['Tax_percent'] if self.kvpairs['Tax_percent'] is not None else 0)))
            if self.kvpairs['Tax_qualifier'] is None: raise MissingValueError("Tax qualifier is None")
        else:
            self.kvpairs['Tax_qualifier'] = (Json.read(self.Config.get('path_five'))).get('Non-EU', {}).get(str(int(self.kvpairs['Tax_percent'] if self.kvpairs['Tax_percent'] is not None else 0)))
            if self.kvpairs['Tax_qualifier'] is None: raise MissingValueError("Tax qualifier is None")
